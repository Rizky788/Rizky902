/*
]=====> RIZKY ID <=====[ ]=====> YT Rizky ID <=====[ ]=====> 085892935752 <=====[
*/
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:RIZKY ID\n' // GANTI NAMA LU
            + 'ORG:OWNER BOTZ;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6285892714463:+62 858-9271-4462\n' // GANTI NOMOR LU
            + 'END:VCARD'
// UDAH SEGITU KONSOL KEBAWAH BIARIN AJA
const ngonsol = JSON.parse(fs.readFileSync('./settings/Rizky.json'))
const {
    botName,
    ownerName,
    BarBarKey,
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' SUBSCRIBE YT RIZKY CHANNEL'))
})

	baby.on('credentials-updated', () => {
		fs.writeFileSync('./Rizky.json', JSON.stringify(baby.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'ingfokan cuyy...')
	})
	fs.existsSync('./Rizky.json') && baby.loadAuthInfo('./Rizky.json')
	baby.on('connecting', () => {
		start('2', 'Rizky Connecting...')
	})
	baby.on('open', () => {
		success('2', 'Rizky Connected')
	})
...
*/
				...
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis1 Rizky baik hati`)
				rizky = body.slice(8)
				reply('「❗」WAIT BRO GUE NULIS DUMLU YAKAN')
				buff = await getBuffer(`https://api.xteam.xyz/magernulis2?text=${rizky}&APIKEY=${XteamKey}`)
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis2 Rizky baik hati`)
				laysha = body.slice(8)
				reply('「❗」WAIT BRO GUE NULIS DUMLU YAKAN')
				...
				baby.sendMessage(from, ''+tes+'\n\n_By : ⸸Rizky⸸Panutanque._', text, { quoted: mek })
				await limitAdd(sender)
				break				
				case 'ninjalogo':
                  if (isBanned) return reply(ind.baned())
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}ninjalogo Rizky & Gans`)
				reply(ind.wait())
		...
		case 'halloweentext':
		    ...
		    break
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}pornhub Rizky & Hub`)
				reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gem = gh.split("&")[0];
					var bok = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}gemboktext 11 01 2021 & Rizky dan Nadia`)
					reply(ind.wait())
				...
				if (!isPrem) return reply(ind.premium())
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}glitchtext Rizky & Gans`)
					reply(ind.wait())
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return baby.sendMessage(from, 'Kode bahasanya mana kak? contoh : ${prefix}tts id Halo Rizky', text, {quoted: mek})
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apaan, titit?\n\ncontoh : ${prefix}bikinquote aku bukan boneka & Kata Rizky`
...
case 'neontext':
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}neontext  Rizky`)
neon = body.slice(10)
reply('「❗」WAIT GANS')
...
case 'blackpink':
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}blackpink Rizky`)
pink = body.slice(11)
...
*/
                ...
                addKoinUser('6285892935752@s.whatsapp.net', fee)
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner Rizky`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`karena Rizky baik Bot memberikan ${one}Xp >_<`)
                      }else{
...
case 'cloudtext':
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}cloudtext Rizkys`)
cloud = body.slice(11)
...
*/
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							baby.sendMessage(_.jid, buff, image, {caption: `*「 BLACK BOT BROADCAST 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 BLACK BOT BROADCAST 」*\n\n${body.slice(4)}`)
						}
						reply('*「 SUKSES BOSKU 」*')
					}
					break
